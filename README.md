...
3. Create a file application-local.properties in /backend/src/main/resources/ with the following variables EMAIL_USERNAME & EMAIL_PASSWORD variables with an gmail account value & 16 character app password value generated by the provided google account. Do not commit this file to the repository
4. To compile the backend:
```bash 
mvn clean install -U
```
